...
async function notifyUpdateIfNeeded(info) {
  try {
    ...
    if (lastNotifiedVersion === info.latestVersion) return;

    let lang = 'en';
    try {
      const settings = await getSettings();
      lang = settings.language === 'fa' ? 'fa' : 'en';
    } catch (e) {
      lang = 'en';
    }

    const label = info.latestLabel || info.latestVersion;
    const message = lang === 'fa'
      ? `نسخه جدید v${label} آماده است. برای مشاهده کلیک کنید.`
      : `New version v${label} is available. Click to view the release.`;
...
function normalizeRecoveryObject(recovery) {
  ...
  if (recovery.wrappedKey) normalized.wrappedKey = String(recovery.wrappedKey);
  if (recovery.wrapIv) normalized.wrapIv = String(recovery.wrapIv);
  return normalized;
}
...
function normalizeLockObject(lock) {
  ...
  return {
    ...
    contentIv: lock.contentIv ? String(lock.contentIv) : '',
    wrappedKey: lock.wrappedKey ? String(lock.wrappedKey) : '',
    wrapIv: lock.wrapIv ? String(lock.wrapIv) : '',
