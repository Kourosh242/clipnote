...
(function () {
  ...
  const els = {
    ...
    btnNormalView: document.getElementById('btn-normal-view'),
    ...
    btnRelockNote: document.getElementById('btn-relock-note'),
  ...
  async function init() {
    ...
    listMode = settings.fullViewMode || 'normal';
  ...
  function applyLocale() {
    ...
    document.getElementById('duplicate-btn-label').textContent = t('duplicate');
    ...
    document.getElementById('relock-label').textContent = t('relock');
    ...
    els.btnImport.textContent = t('import');
    els.btnExport.textContent = t('export');
    els.btnNormalView.textContent = t('normalView');
    els.btnTimelineView.textContent = t('timelineView');
    ...
    els.btnConfirmUnlock.textContent = t('unlockNote');
  ...
  function setupEventListeners() {
    ...
    els.btnNormalView.addEventListener('click', () => setListMode('normal'));
    els.btnTimelineView.addEventListener('click', () => setListMode('timeline'));
    ...
    els.btnRelockNote.addEventListener('click', relockCurrentNoteSession);
  ...
  function renderNotesList() {
    ...
    els.notesGrid.classList.toggle('timeline-mode', listMode === 'timeline');
    ...
    if (listMode === 'timeline') {
      renderTimelineNotes(notes);
      return;
    }
    notes.forEach(note => els.notesGrid.appendChild(buildNoteCard(note)));
  }

  function renderTimelineNotes(notes) {
    getTimelineGroups(notes, (items) => items).forEach(group => {
      const section = document.createElement('section');
      section.className = 'cn-timeline-group';
      section.innerHTML = `
      ...
      grid.className = 'cn-timeline-grid';
      group.notes.forEach(note => grid.appendChild(buildNoteCard(note)));
  ...
  function renderLockState() {
    ...
    els.btnRelockNote.classList.toggle('cn-hidden', !locked);
  }
  ...
  async function clearAllData() {
    ...
    isDirty = false;
    unlockedNotes.clear();
    listMode = settings.fullViewMode;
    await applyTheme(settings);
    renderAllListState();
    loadSettingsUI();
    await showView('list');
  ...
  async function setListMode(mode) {
    listMode = mode;
    settings.fullViewMode = mode;
    await saveSettings({ ...settings, fullViewMode: mode });
    applyListModeUI();
    renderNotesList();
  }

  function applyListModeUI() {
    els.btnNormalView.classList.toggle('active', listMode === 'normal');
    els.btnTimelineView.classList.toggle('active', listMode === 'timeline');
  }
